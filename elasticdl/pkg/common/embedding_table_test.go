...
func TestEmbeddingTableGet(t *testing.T) {
	...
	assert.Equal(t, Slice(v1).([]float32), []float32{1, 2})
...
func TestEmbeddingTableSet(t *testing.T) {
	...
	assert.True(t, CompareFloatArray([]float32{1.0, 2.0}, Slice(v1).([]float32), 0.0001))

	v3 := e.GetEmbeddingVector(3)
	assert.True(t, CompareFloatArray([]float32{3.0, 4.0}, Slice(v3).([]float32), 0.0001))

	v5 := e.GetEmbeddingVector(5)
	assert.True(t, CompareFloatArray([]float32{5.0, 6.0}, Slice(v5).([]float32), 0.0001))
}
